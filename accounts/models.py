...
class MailUser(models.Model):
    ...
    def _get_digest(self, raw_password, salt):
        ...
        m = hashlib.sha1()
        m.update(str(raw_password).encode('utf-8'))
        m.update(str(salt).encode('utf-8'))
        digest = base64.b64encode(m.digest() + str(self.salt).encode('utf-8')).decode('utf-8')
        return digest
    ...
    def check_password(self, raw_password):
        ...
        """
        new = self._get_digest(raw_password, self.salt)
        old = self.shadigest
        if old == new:
            return True
        else:
...
class Alias(models.Model):
    ...
    """
    domain = models.ForeignKey(Domain, help_text='Domain owning the alias.', on_delete=models.CASCADE)
